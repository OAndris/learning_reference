# Source: https://www.linkedin.com/learning/python-essential-training-2/a-database-interface

"""
NOTE:
This file contains a custom made database wrapper class.
There are also multiple publicly available ready-to-use solutions.
However, sometimes it makes sense to write your own so that it contains only what you need and does exactly what you want it to do,
without the extra added complexity and potentially reduced efficiency.
"""
...
class DatabaseWrapper:
    ...
    def insert_nocommit(self, rec):
        ...
        values = [rec[k] for k in klist]  # a list of values ordered by key
...
def test(file_based_database=False):
    if file_based_database:
        fn = 'database_wrapper.db'
        try:
            os.stat(fn)
        except:
            pass
        ...
        fn = ':memory:'  # special string specific to SQLite, resulting in an in-memory database, thus not creating an actual file
    ...
    print('File version:', __version__)
    
    print(f'Create database file "{fn}" ... ', end='')
    db = DatabaseWrapper(filename=fn, table=t)
    print('Done.')
    
    print(f'Create table ... ', end='')
